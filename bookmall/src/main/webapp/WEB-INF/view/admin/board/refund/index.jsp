	                        ...
	                        <form name="askspace" id="askspace">
		                        <c:if test="${!empty refundList }">
			                           <c:forEach var="refund" items="${refundList }" varStatus="status">
										<tr>
											<td class="first"><input type="checkbox" name="refundno" id="check" value="${refund.refundno }"></td>
											<td>${refund.division}</td>
											<td>${refund.orderno}</td>
											<td>${refund.reason}</td>
											<td><b>${refund.receiver_name } </b><br>
												${refund.zipcode }<br>
												${refund.addr1}  ${refund.addr2}
											</td>
											<td>${refund.tel1} - ${refund.tel2} - ${refund.tel3} </td>
											<td>${refund.creditCard }
											<fmt:formatNumber value="${refund.paid_amount }" pattern="###,###" />원</td>
											<td><fmt:formatDate value="${refund.order_date }" pattern="yyyy-MM-dd" /></td>
											<td><fmt:formatDate value="${refund.regdate }" pattern="yyyy-MM-dd" /></td>
											<td>${refund.exchangeno }</td>
											<td></td>
										</tr>
			                            </c:forEach>
		                        </c:if>    
