	...
	$.ajax ({
		...
		success:function(res) {
			...
			for(var i=0; i < res.length; i++) {
				html += '<span id="bcategoryno_'+res[i].bcategoryno+'" value="'+res[i].bcategoryno+'" onclick="category_two('+res[i].bclassify+', '+res[i].bcategoryno+', \''+res[i].bcategoryname+'\');">'+res[i].bcategoryname+"</span><br>"
			}
	...
	$.ajax ({
		...
		success:function(res) {
			...
			if($("#bcategoryno2_"+bparentno).length) {
				...
				select += '<span id="bcategoryno2_'+bparentno+'"><input type="hidden" name="bparentnoS"  value="'+bparentno+'">'+bcategoryname+'<button onclick="delcate('+bparentno+')">&nbsp;X</button><br></span>'
				//select +=
			}
...
function category_three(bparentno, bcategoryno, bcategoryname) {
	...
	if($("#bcategoryno2_"+bcategoryno).length) {
		...
		select += '<span id="bcategoryno2_'+bcategoryno+'"><input type="hidden" name="bcategorynoS"  value="'+bcategoryno+'">'+bcategoryname+'<button onclick="delcate('+bcategoryno+')">&nbsp;X</button><br></span>'
			...
			<div id="content">
				<div class="con_tit">
                	<div class="bbs">				
						...
						<!-- css 검색 -->
							...
							<div class="">
								...
								<div id="category_all" class="category_all">
									<!-- 전체분류 -->
									<div id="category_one" class="category_one" style="float : left; margin-right: 30px;">
										<span onclick="category_one(${vo.korea});">국내 도서</span><br>
										<span onclick="category_one(${vo.foreign});">외국 도서</span><br>
									</div>
									
									<!-- 대분류 -->
									<div id="category_two" class="category_two" style="float : left; margin-right: 30px; overflow: auto; width: 200px; height: 300px;">
										<!-- 전체 분류 미 선택 시  -->
										<div id="category_two_defalt" class="category_two_defalt" >
											<span >국내, 외국 분류를 선택 해 주세요</span>
										</div>
										<!-- 전체 분류 선택 시 -->
										<div id="category_two_select" class="category_two_select" >
												<span onclick="category_two();"></span><br>
										</div>
									</div>
									
									<!-- 중분류 -->
									<div id="category_three" class="category_three" style="float : left; margin-right: 30px; overflow: auto; width: 200px; height: 300px;">
										<!-- 전체 분류 미 선택 시  -->
										<div id="category_three_defalt" class="category_three_defalt" >
											<span >대분류를 선택 해 주세요</span>
										</div>
										<!-- 전체 분류 선택 시 -->
										<div id="category_three_select" class="category_three_select" >
						...
						<form name="searchForm_Condition" class="searchForm_Condition" id="searchForm_Condition" action="#"  method="get">
								<dl>
									<dt>
										<input id="initialization" type="reset" value="초기화">
									</dt>
									<dd id="searchForm_Condition_Add">
